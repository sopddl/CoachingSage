                ...
                VStack(spacing: 16) {
                    if phase.kind == .warmup || phase.kind == .cooldown {
                        ...
                        if let reps = strengthRepTarget {
                            ...
                            VStack(spacing: 6) {
                                // Bloc héros extrait (StrengthRepsHero) → rendable seul en snapshot
                                // FR/EN/ES (filet visuel : le gros chiffre = `value` propre, pas
                                // « 10 par jambe ». Cf StrengthRepsHeroSnapshotTests).
                                StrengthRepsHero(value: heroValue, isLateral: heroIsLateral)
