...
rows = []
for path in sorted(glob.glob(os.path.join(TPL_DIR, "hiking-*.json"))):
...
json.dump(uniq, open("hiking_audit/corpus.json","w"), ensure_ascii=False, indent=1)
...
open("hiking_audit/exo_names.txt","w").write("\n".join(names))
