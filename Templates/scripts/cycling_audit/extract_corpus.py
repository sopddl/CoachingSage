...
rows = []
for path in sorted(glob.glob(os.path.join(TPL_DIR, "cycling-*.json"))):
...
json.dump(uniq, open("cycling_audit/corpus.json","w"), ensure_ascii=False, indent=1)
...
open("cycling_audit/exo_names.txt","w").write("\n".join(names))
